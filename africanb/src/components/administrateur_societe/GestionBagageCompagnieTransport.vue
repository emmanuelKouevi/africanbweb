        ...
        <v-card width="1200" elevation="3">
          ...
          <v-container>
            <v-row>
              <v-col cols="5">
                <v-text-field
                  :error-messages="designationBagage"
                  rounded
                  dense
                  outlined
                  label="Réference du sac"
                  v-model.trim="$v.bagageModel.designation.$model"
                ></v-text-field>
              </v-col>
              <v-col cols="5">
                <v-select
                  :error-messages="typeBagageErrors"
                  :items="referenceBagageList"
                  ...
                  outlined
                  color="teal"
                  label="sélectionnez un type de bagage"
                  v-model.trim="$v.bagageModel.typeBagageDesignation.$model"
                ></v-select>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="3">
                <v-text-field
                  :error-messages="nombreBagageErrors"
                  rounded
                  dense
                  outlined
                  label="nombre de sac gratuit"
                  v-model.trim="
                    $v.bagageModel.nombreBagageGratuitParTypeBagage.$model
                  "
                ></v-text-field>
              </v-col>
              <v-col cols="5">
                <v-text-field
                  :error-messages="prixBagageErrors"
                  rounded
                  dense
                  outlined
                  color="teal"
                  label="prix par type de sac"
                  suffix="FCFA"
                  v-model.number="$v.bagageModel.coutBagageParTypeBagage.$model"
                ></v-text-field>
              </v-col>
            </v-row>
          </v-container>
...
export default {
  ...
  methods: {
    ...
    async creerBagage() {
      ...
      this.overlay = true;
      await axios
        .post(API_CREER_BAGAGE, this.bagageToSend, {
  ...
  computed: {
    ...
    designationBagage() {
      ...
      !this.$v.bagageModel.designation.required &&
    ...
    prixBagageErrors() {
      ...
      !this.isNumber(this.bagageModel.coutBagageParTypeBagage) &&
        errors.push("Saisissez un prix correct.");
      !this.$v.bagageModel.coutBagageParTypeBagage.required &&
    ...
    typeBagageErrors() {
      ...
      !this.$v.bagageModel.typeBagageDesignation.required &&
    ...
    nombreBagageErrors() {
      const errors = [];
      !this.isNumber(this.bagageModel.nombreBagageGratuitParTypeBagage) &&
        errors.push("Saisissez un nombre correct.");
      if (!this.$v.bagageModel.nombreBagageGratuitParTypeBagage.$dirty)
        return errors;
      !this.$v.bagageModel.nombreBagageGratuitParTypeBagage.required &&
