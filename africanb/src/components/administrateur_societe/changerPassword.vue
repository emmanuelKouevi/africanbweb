  ...
  <v-app>
    <v-form @submit.prevent="submitForm">
      <v-card class="mx-auto" max-width="700">
        ...
        <v-card-subtitle
          >Changer votre de passe pour une meilleur sécurité</v-card-subtitle
        >
        <v-card-text>
          <v-container>
            <v-row>
            ...
            <v-row>
              <v-col>
                <v-text-field
                  ...
                  v-model.trim="$v.userParams.oldPassWord.$model"
                  :append-icon="
                    oldPasswordIsVisible ? 'mdi-eye' : 'mdi-eye-off'
                  "
                  class="input_field myInput"
                  @click:append="oldPasswordIsVisible = !oldPasswordIsVisible"
              ...
              <v-col>
                <v-text-field
                  ...
                  v-model.trim="$v.userParams.newPassWord.$model"
                  :append-icon="
                    newPasswordIsVisible ? 'mdi-eye' : 'mdi-eye-off'
                  "
                  class="input_field myInput"
                  @click:append="newPasswordIsVisible = !newPasswordIsVisible"
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      this.$v.$touch();
      if (this.$v.userParams.$invalid)
        this.errorMsg = "Vous avez omis des champs de saisie";
    ...
    async resetPasswordUser() {
      ...
      await axios
        .post(API_RESET_PASSWORD_USER, this.userReset, {
  ...
  computed: {
    ...
    oldPasswordError() {
      ...
      !this.$v.userParams.oldPassWord.required &&
        errors.push("Entrer l'ancien mot de passe.");
      return errors;
    },
    ...
    newPasswordError() {
      ...
      !this.$v.userParams.newPassWord.required &&
