    ...
    <v-form @submit.prevent="submitForm">
      <v-container fluid>
        <v-card max-width="800px" class="mx-auto">
          <v-card-title
            ><h4>CREATION DE RÔLE UTILISATEUR</h4>
            <v-spacer></v-spacer>
            <v-dialog transition="dialog-top-transition" max-width="700">
              <template v-slot:activator="{ on, attrs }">
                <v-btn color="white" rounded small v-bind="attrs" v-on="on"
                  >DEFINIR UN RÔLE</v-btn
                >
              </template>
              <template v-slot:default="dialog">
                <v-card>
                  <v-toolbar color="white"
                    ><span class="font-weight-bold">PARAMETRES DES ROLES</span>
                    <v-spacer></v-spacer>
                    <v-dialog
                      v-model="viewingFunctionnalityDialog"
                      max-width="350"
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-badge
                          :content="functionnalitiesChoosenList.length"
                          :value="functionnalitiesChoosenList.length"
                          color="primary"
                          overlap
                        >
                          <v-btn small text v-bind="attrs" v-on="on"
                            >VOIR LES FONCTIONNALITÉS</v-btn
                          >
                        </v-badge>
                      </template>
                      <v-card>
                        <v-card-text
                          v-if="functionnalitiesChoosenList.length > 0"
                        >
                          <v-list two-line>
                            <v-list-item-group multiple>
                              <template>
                                <v-list-item
                                  v-for="(
                                    item, index
                                  ) in functionnalitiesChoosenList"
                                  :key="item.title"
                                >
                                  <template>
                                    <v-list-item-content>
                                    ...
                                    <v-list-item-action
                                      @click="deleteFunctionnalityInList(index)"
                                    >
                                ...
                                <v-divider
                                  v-if="
                                    index <
                                    functionnalitiesChoosenList.length - 1
                                  "
                                  :key="index"
                                ></v-divider>
                              </template>
                        ...
                        <v-card-text v-else> AUCUN CODE DÉFINI </v-card-text>
                      </v-card>
                    </v-dialog>
                    <v-btn
                      text
                      small
                      color="black"
                      @click="addUserRole(userRoleObject)"
                      ><v-icon>mdi-check</v-icon>VALIDER</v-btn
                    >
                  </v-toolbar>
                  <v-card-text>
                    <v-container fluid>
                      <v-subheader
                        ><v-icon color="#40407a">mdi-alert-circle</v-icon
                        >&nbsp;&nbsp;
                        <v-spacer></v-spacer>
                        <v-dialog v-model="functionalityDialog" max-width="380">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn small outlined v-bind="attrs" v-on="on"
                              >Fonctionnalités à rattacher</v-btn
                            >
                          </template>
                          <v-card>
                            <v-card-title class="text font-weight-thin"
                              >FONCTIONNALITÉS DISPONIBLES</v-card-title
                            >
                            <v-card-text>
                              <v-select
                                dense
                                v-model="functionnalitiesChoosenList"
                                :items="getFunctionalitiesList"
                                item-text="code"
                                item-value="code"
                                :menu-props="{ maxHeight: '400' }"
                                label="Vos fonctionnalités"
                                multiple
                                hint="Sélectionnez vos fonctionnalités"
                                persistent-hint
                              ></v-select>
                            </v-card-text>
                            <v-card-actions>
                              <v-spacer></v-spacer>
                              <v-btn text @click="functionalityDialog = false"
                                >Annuler</v-btn
                              >
                            </v-card-actions>
                          </v-card>
                      ...
                      <v-subheader
                        ><br />Veuillez renseigner les champs :
                        &nbsp;&nbsp;&nbsp;
                      </v-subheader>
                      <v-row>
                        <v-col
                          ><v-text-field
                            class="my_input"
                            v-model="userRoleObject.libelle"
                            rounded
                            dense
                            outlined
                            label="Libellé"
                        ...
                        <v-col
                          ><v-text-field
                            class="my_input"
                            :error-messages="codeContrainte"
                            v-model.trim="$v.userRoleObject.code.$model"
                            rounded
                            dense
                            outlined
                            label="Code"
            ...
            <v-row v-if="userRoleDataToSend.datas.length == 0">
              <v-col class="text-center">
                <span class="font-weight-thin"
                  >AUCUN RÔLE UTILISATEUR DÉFINI</span
                >
              </v-col>
            </v-row>
            ...
            <v-row v-else>
              <v-sheet width="750" class="mx-auto">
                <v-card-title class="title-card"
                  ><span><h5>RÔLES UTILISATEURS</h5></span>
                  <v-spacer></v-spacer>
                  <v-text-field
                    v-model="search"
          ...
          <v-card-actions>
            ...
            <v-btn type="submit" small shaped rounded outlined color="primary"
              >SAUVEGARDER</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  computed: {
    ...
    codeContrainte() {
      ...
      !this.$v.userRoleObject.code.required &&
