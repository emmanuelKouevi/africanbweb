<template>
  <v-app>
    <v-card class="mb-5" elevation="5" rounded="lg"
      ><br />
      <v-row justify="center">
        <v-col class="text-center">
          <span class="subtitle font-weight-thin">OFFRE DE VOYAGE :</span>
          <span class="offreVoyageDesignation font-weight-bold">
      ...
      <v-row>
          ...
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle class="subtitle"
                >Total Programmation:</v-list-item-subtitle
              >
            </v-list-item-content>

            <v-list-item-content>
              <v-list-item-subtitle>{{
                jourSemainesParOffreVoyagesList.length
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
        ...
        <v-col>
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle class="subtitle"
                >Total Mode de financement :</v-list-item-subtitle
              >
            </v-list-item-content>

            <v-list-item-content>
              <v-list-item-subtitle>{{
                prixEtModeParOffreVoyageList.length
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
      ...
      <v-row>
        <v-col class="text-center">
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle class="subtitle"
                >Total Propriétés :</v-list-item-subtitle
              >
            </v-list-item-content>

            <v-list-item-content>
              <v-list-item-subtitle>{{
                proprieteParOffreVoyageList.length
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
        ...
        <v-col>
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle class="subtitle"
                >Total villes Escales :</v-list-item-subtitle
              >
            </v-list-item-content>

            <v-list-item-content>
              <v-list-item-subtitle>{{
                villesEscalesParOffreVoyagesList.length
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
      ...
      <v-row>
        <v-col cols="6" class="text-center">
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle class="subtitle"
                >Total Bus :</v-list-item-subtitle
              >
            </v-list-item-content>

            <v-list-item-content>
              <v-list-item-subtitle>{{
                busParOffreVoyageList.length
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
    ...
    <v-form>
      <v-card>
        <v-card-title class="title_card"
          >INFORMATION(S) GÉNÉRALE(S) DE L'OFFRE DE VOYAGE
          <v-spacer></v-spacer>
          <v-btn
            outlined
            rounded
            ...
            small
            title="Voir plus"
            @click="isVisibleOffreVoyageInfo()"
      ...
      <v-card :loading="loadingModePrix">
        <v-card-title class="title_card"
          ...
          <v-btn
            ...
            small
            title="Voir plus"
            @click="isVisiblePrixModeOffreVoyage()"
            ><v-icon>mdi-menu-down</v-icon></v-btn
          >
        </v-card-title>
        <v-card id="card-prix-mode-offre-voyage">
          <v-container>
            <v-row v-if="prixEtModeParOffreVoyageList.length">
              <v-col
                cols="6"
                v-for="(mode, index) in prixEtModeParOffreVoyageList"
                :key="index"
              >
                <v-card>
                  <v-card-title
                    >Mode n° {{ index + 1 }}
                    <v-spacer></v-spacer>
                    <v-btn icon @click="editerModeOffreVoyage(mode)"
                  ...
                  <v-container>
                    <v-row>
                      <v-col cols="3"
                        ><small class="muted-text">Désignation :</small></v-col
                      >
                      <v-col
                    ...
                    <v-row>
                      <v-col cols="3"
                        ><small class="muted-text"
                          >Mode de l'offre :</small
                        ></v-col
                      >
                      <v-col
                        ><span class="label-text">{{
                          mode.modeDesignation
                        }}</span></v-col
                      >
                    </v-row>
                    ...
                    <v-row>
                      <v-col cols="3"
                        ><small class="muted-text">Catégorie :</small></v-col
                      >
                      <v-col
                        ><span class="label-text">{{
                          mode.categorieVoyageurDesignation
                        }}</span></v-col
                      >
                    </v-row>
      ...
      <v-card :loading="loadingProprieteOffreVoyage">
        <v-card-title class="title_card"
          ...
          <v-btn
            ...
            small
            title="Voir plus"
            @click="isVisibleProprieteOffreVoyage()"
            ><v-icon>mdi-menu-down</v-icon></v-btn
          >
        </v-card-title>
        <v-card id="card-propriete-offre-voyage">
          <v-container>
            <v-row>
              <v-col
                cols="6"
                v-for="(propriete, index) in proprieteParOffreVoyageList"
                :key="index"
              >
                <v-chip color="teal" text-color="white" class="mr-2"
                  ><span class="etat">{{ propriete.designation }}</span></v-chip
                >
              </v-col>
            </v-row>
          </v-container>
      ...
      <v-card :loading="loadingProgrammeOffreVoyage">
        <v-card-title class="title_card"
          ...
          <v-btn
            ...
            small
            title="Voir plus"
            @click="isVisibleProgrammeOffreVoyage()"
            ><v-icon>mdi-menu-down</v-icon></v-btn
          >
        </v-card-title>
        <v-card id="card-programme-offre-voyage">
          <v-container>
            <v-row>
              <v-col
                cols="6"
                v-for="(jourSemaine, index) in jourSemainesParOffreVoyagesList"
                :key="index"
              >
                <v-card>
                  <v-card-title
                    >Programme n° {{ index + 1 }}
                    <v-spacer></v-spacer>
                    <v-btn icon @click="editerJourOffre(jourSemaine)"
                      ><v-icon color="primary">mdi-pencil</v-icon></v-btn
                    >
                    <v-btn icon><v-icon color="red">mdi-close</v-icon></v-btn>
                  </v-card-title>
                  <v-container>
                    <v-row>
                      ...
                      <v-col
                        ><span class="label-text">{{
                          jourSemaine.designation
                        }}</span></v-col
                      >
                    </v-row>
                    <v-row>
                      <v-col cols="3"
                        ><small class="muted-text">Jour :</small></v-col
                      >
                      <v-col
                        ><span class="label-text">{{
...
import {
  ...
  API_RECUPERER_PROPRIETE_PAR_OFFRE_VOYAGE,
  API_RECUPERER_LISTE_BUS_PAR_OFFRE_VOYAGE,
} from "../globalConfig/globalConstConfig";
...
export default {
  ...
  methods: {
    ...
    async modifierOffreVoyageInfo() {
      ...
      this.overlay = true;
      await axios
        .put(API_CREER_OFFRE_VOYAGE, this.offreVoyageToModify, {
        ...
        .then((response) => {
          if (response.status == 200) {
            ...
            this.warningMsg =
              "Erreur , lors de la modification de l'offre de voyage";
    ...
    async obtenirListeDesVillesDisponible() {
      this.loading = false;
      await axios
        .post(API_OBTENIR_LISTE_DES_VILLES_DISPONIBLE, this.simpleObject, {
          headers: HEADERS,
        })
        .then((response) => {
