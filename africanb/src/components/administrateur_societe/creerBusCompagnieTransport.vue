    ...
    <v-form @submit.prevent="submitForm">
      <v-container fluid>
        <v-card max-width="800px" class="mx-auto">
          <v-card-title
            ><h4>ASSOCIER DES BUS À UNE OFFRE DE VOYAGE</h4>
            <v-spacer></v-spacer>
            <v-dialog transition="dialog-top-transition" max-width="700">
              <template v-slot:activator="{ on, attrs }">
                <v-btn color="white" rounded small v-bind="attrs" v-on="on"
                  >AJOUTER UN BUS</v-btn
                >
              </template>
              <template v-slot:default="dialog">
                <v-card>
                  <v-toolbar color="white"
                    ><span class="font-weight-bold">INFOS DU BUS</span>
                    <v-spacer></v-spacer>
                    <v-btn
                      text
                      small
                      color="black"
                      @click="enregistrerBus(addedBusData)"
                  ...
                  <v-card-text>
                    <v-container fluid>
                      <v-subheader
                        ...
                        >&nbsp;&nbsp;Le nombre de place et le numéro du bus
                        doivent être au format numerique</v-subheader
                      >
                      <v-subheader
                        ><br />Définissez les références de vos bus :
                        &nbsp;&nbsp;&nbsp;
                      </v-subheader>
                      <v-row>
                        <v-col
          ...
          <v-card-subtitle
            >Définissez les bus de votre offre de voyage</v-card-subtitle
          >
          <v-container>
            <v-row>
              <v-col cols="4">
                <v-select
                  class="my_input"
                  :error-messages="offreVoyageDesignationContainte"
                  v-model.trim="$v.addedBusData.offreVoyageDesignation.$model"
            ...
            <v-row>
              <v-sheet width="750" class="mx-auto">
                <v-card-title class="title-card"
                  ><span><h5>BUS DISPONIBLES OFFRE DE VOYAGE</h5></span>
                  <v-spacer></v-spacer>
                  <v-text-field
                    v-model="search"
          ...
          <v-card-actions>
            ...
            <v-btn type="submit" small shaped rounded outlined color="#474787"
              >SAUVEGARDER</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    enregistrerBus(busAdded) {
      this.$v.$touch();
      if (
        this.$v.addedBusData.designation.$invalid ||
        this.$v.addedBusData.numeroBus.$invalid ||
  ...
  computed: {
    ...
    offreVoyageDesignationContainte() {
      ...
      !this.$v.addedBusData.offreVoyageDesignation.required &&
