  ...
  <v-app>
    <v-form @submit.prevent="submitForm">
      <v-container fluid>
        <v-card>
          <v-card-title
            >GESTION DES VILLES EN ESCALES
            <v-spacer></v-spacer>
            <v-dialog transition="dialog-top-transition" max-width="700">
              <template v-slot:activator="{ on, attrs }">
                <v-btn color="secondary" rounded small v-bind="attrs" v-on="on"
                  >Ajouter des villes escales &nbsp;<v-icon
                ...
                <v-card>
                  <v-toolbar color="white"
                    ><span class="font-weight-bold"
                      >PARAMÈTRAGE DES VILLES D'ESCALES</span
                    ></v-toolbar
                  >
                  <v-card-text>
                    <v-container fluid>
                      <v-subheader
                        ><br />Définissez le nombre de villes d'escales :
                        &nbsp;&nbsp;&nbsp;
                      </v-subheader>
                      <v-row>
                        <v-col cols="3"
                          ><v-text-field
                            ...
                            type="number"
                            min="0"
                            v-model="villeEscaleData.position"
                            dense
                            outlined
                            rounded
                          ></v-text-field
                        ...
                        <v-col cols="5"
                          ><v-select
                            label="Villes escales"
                            :items="villesList"
                            item-text="designation"
                            v-model="villeEscaleData.villeDesignation"
                        ...
                        <v-col cols="3"
                          ><v-btn
                            icon
                            @click="validerVilleEscale(villeEscaleData)"
          ...
          <v-card-subtitle
            >Gérer les villes escales pour une offre</v-card-subtitle
          >
          <v-container>
            <v-row>
              <v-col cols="6">
                <v-select
                  ...
                  v-model.trim="
                    $v.villeEscaleData.offreVoyageDesignation.$model
                  "
                  dense
                  outlined
                  rounded
                ...
                >
                  ...
                  <template v-slot:[`item.actions`]="{ item }">
                    <v-icon
                      ...
                      small
                      class="mr-2"
                      @click="supprimerVilleEscale(item)"
                      >mdi-delete</v-icon
                    >
                  </template>
                </v-data-table>
          ...
          <v-card-actions>
            ...
            <v-btn type="submit" rounded outlined small color="primary"
              >APPLIQUER</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    async creerVillesEscaleOffreVoyage() {
      this.overlay = true;
      await axios
        .post(API_CREER_VILLES_ESCALES_OFFRE_VOYAGE, this.villesObject, {
    ...
    validerVilleEscale(villeEscale) {
      ...
      if (
        ...
        villeEscaleSaisi.offreVoyageDesignation =
          villeEscale.offreVoyageDesignation;
  ...
  computed: {
    ...
    offreVoyageDesignation() {
      ...
      !this.$v.villeEscaleData.offreVoyageDesignation.required &&
