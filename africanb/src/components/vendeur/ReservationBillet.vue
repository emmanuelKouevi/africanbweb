                ...
                <v-card-text>
                  <form class="row g-3">
                    ...
                    <div class="col-md-3">
                      ...
                      <input
                        type="text"
                        v-model="programmeReserved.data.nombrePlace"
                        class="form-control"
                        id="inputZip"
                      />
                    </div>
            ...
            <v-card-text>
              <v-list>
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-title class="font-weight-bold"
                      >RESERVATION EN COURS:</v-list-item-title
                    >
                  </v-list-item-content>
                </v-list-item>
                ...
                <v-list-item>
                  ...
                  <v-list-item-content>
                    <v-list-item-subtitle class="font-weight-bold">{{
                      modeChoosen.categorieVoyageurDesignation
                    }}</v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
                ...
                <v-list-item>
                  <v-list-item-content>
                    <v-list-item-subtitle
                      >Numéro de télephone :</v-list-item-subtitle
                    >
                  </v-list-item-content>
                ...
                <v-list-item>
                  ...
                  <v-list-item-content>
                    <v-list-item-subtitle class="font-weight-bold">{{
                      programmeReserved.data.nombrePlace
                    }}</v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </v-list>
      ...
      <v-sheet>
        <v-toolbar
          ><v-toolbar-title
            ><span class="font-weight-bold"
              >CONFIRMATION DE RESERVATION</span
            ></v-toolbar-title
          ></v-toolbar
        ><br />
        ...
        <v-card>
              ...
              <v-card>
                ...
                <v-card-text>
                  <v-container>
                    ...
                    ><br /><br />
                    <v-row>
                      <v-icon dense color="teal">mdi-circle</v-icon>&nbsp;&nbsp;
                      Gare départ :
                      <span class="gareDepart">{{
                        offreVoyage.villeDepartDesignation
                      }}</span> </v-row
                    ><br /><br />
                    <v-row>
                      <v-icon dense color="#2C3A47">mdi-circle</v-icon
                      >&nbsp;&nbsp; Gare destination :
                      <span class="gareDestination"
                        >{{ offreVoyage.villeDestinationDesignation }}
                      </span>
                    </v-row>
                  </v-container>
              ...
              <v-card>
                ...
                <v-card-text>
                  <v-container>
                    <v-row>
                      <span>Nom:</span>&nbsp;&nbsp;
                      <span>{{ user.nom }}</span> </v-row
                    ><br /><br />
                    <v-row>
                      <span>Prenoms:</span>&nbsp;&nbsp;
                      <span>{{ user.prenoms }}</span> </v-row
                    ><br /><br />
                    <v-row>
                      <span>Contact:</span>&nbsp;&nbsp;
                ...
                <v-card-text>
                  <v-container>
                    <div class="form-group">
                      ...
                      <v-select
                        class="col-sm-6"
                        :items="prixEtModeParOffreVoyageList"
                        item-text="categorieVoyageurDesignation"
                        item-value="categorieVoyageurDesignation"
                        v-model="programmeReserved.data.categorieVoyageur"
                        dense
                        outlined
                      ></v-select>
                    </div>
                    <div>
                      ...
                      <span class="data-label">{{ getModeByOffre }}</span
                      ><br /><br />
                    </div>
                    <div>
                      <span class="font-weight-bold">Montant à payer :</span>
                      <span class="data-label font-weight-bold">{{
                        getPrixByOffre
                      }}</span>
                      FCFA
                    </div>
          ...
          <v-card-actions>
            ...
            <v-btn x-small color="success" @click="reserverOffre()"
              >confirmer</v-btn
            >
          </v-card-actions>
        </v-card>
...
import {
  API_RESERVER_PROGRAMME_OFFRE,
  API_CREATE_HISTORIQUE_PAIEMENT,
} from "../globalConfig/globalConstConfig";
export default {
  ...
  methods: {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            this.programmeReserved.data.categorieVoyageur =
              this.modeChoosen.categorieVoyageurDesignation;
            this.programmeReserved.data.clientDetails = this.user;

            axios
              .post(API_RESERVER_PROGRAMME_OFFRE, this.programmeReserved, {
                headers: HEADERS(this.$store.state.userAuthentified.token),
              })
              .then((response) => {
                if (response.status == 200) {
                  if (response.data.status.code == 800) {
                    this.creerHistoriquePaiement(
                      response.data.items[0].designation
                    );
    ...
    async creerHistoriquePaiement(designationReservation) {
      ...
      this.historyPaiement.data.reservationBilletVoyageDesignation =
        designationReservation;
      this.historyPaiement.data.dateTimePayment = formattedDate;
      this.historyPaiement.data.modePaiementDesignation = "Espéce";
      await axios
        .post(API_CREATE_HISTORIQUE_PAIEMENT, this.historyPaiement, {
          headers: HEADERS(this.$store.state.userAuthentified.token),
        })
        .then((response) => {
          if (response.status == 200) {
  ...
  computed: {
    getProgrammeFromDaySelected() {
      ...
      this.jourSemainesParOffreVoyagesList.forEach((element) => {
        if (this.jourDepart == element.jourSemaineDesignation) {
          if (
            element.programmeDTOList != null ||
            element.programmeDTOList != undefined
          ) {
    ...
    getModeByOffre() {
      ...
      this.prixEtModeParOffreVoyageList.forEach((element) => {
        if (
          this.programmeReserved.data.categorieVoyageur ==
          element.categorieVoyageurDesignation
        ) {
