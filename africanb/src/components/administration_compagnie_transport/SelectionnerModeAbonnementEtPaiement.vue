<template>
  <v-app>
    <v-container>
      ...
      <v-card>
        <v-card-title class="title-card"
          >LISTE DES MODES DE PAIEMENTS</v-card-title
        >
        <v-container fluid>
          <v-row>
            <v-col
              cols="6"
              v-for="(paiement, item) in paiementDisponibleList"
              :key="item"
            >
              <v-card>
                <v-list>
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title class="title_simulation">
                        MODE DE PAIEMENT {{ item + 1 }}</v-list-item-title
                      >
                    </v-list-item-content>
                    ...
                    <v-list-item-action>
                      <v-menu bottom right>
                        ...
                        <v-list>
                          <v-list-item
                            link
                            @click="editerModePaiement(paiement)"
                          >
                            <v-list-item-action
                          ...
                          <v-list-item link>
                            ...
                            <v-list-item-subtitle
                              >Supprimer</v-list-item-subtitle
                            >
                          </v-list-item>
                        </v-list>
                      </v-menu>
                  ...
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-subtitle
                        >Type de paiement :</v-list-item-subtitle
                      >
                    </v-list-item-content>

                    <v-list-item-content>
                      <v-list-item-subtitle>{{
                        paiement.typeModePaiementDesignation
                      }}</v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                </v-list>
...
import $ from "jquery";
import {
  API_OBTENIR_MODE_ABONNEMENT_PAR_COMPAGNIE,
  API_OBTENIR_MODE_PAIEMENT_PAR_COMPAGNIE,
  HEADERS,
