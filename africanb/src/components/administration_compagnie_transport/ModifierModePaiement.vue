        ...
        <v-card width="1200" elevation="3">
          ...
          <v-container>
            <v-row>
              <v-col>
                <v-text-field
                  :error-messages="designationPaiementErrors"
                  rounded
                  dense
                  outlined
                  label="Désignation"
              ...
              <v-col>
                <v-text-field
                  :error-messages="telephoneGeneriqueErrors"
                  rounded
                  dense
                  outlined
                  color="teal"
                  label="Numéro de téléphone"
                  v-model.trim="$v.modePaiementModel.telephoneGenerique.$model"
                ></v-text-field>
              </v-col>
              <v-col>
                <v-select
                  :items="referenceTypePaiementList"
                  item-value="designation"
                  item-text="designation"
                  :error-messages="typePaiementErrors"
                  rounded
                  dense
                  outlined
                  color="teal"
          ...
          <v-card-actions>
            ...
            <v-btn small type="submit" rounded outlined color="primary"
              >ACCEPTER LES MODIFICATIONS</v-btn
            >
          </v-card-actions>
        </v-card>
      </v-container>
...
export default {
  ...
  methods: {
    ...
    async modifierModePaiement() {
      ...
      this.overlay = true;
      await axios
        .post(API_MODIFIER_MODE_PAIEMENT, this.modePaiementToModify, {
          headers: HEADERS(this.$store.state.userAuthentified.token),
        })
        .then((response) => {
    ...
    async editerModePaiement() {
      if (localStorage.getItem("modePaiement")) {
        try {
          const modePaiementEditing = JSON.parse(
            localStorage.getItem("modePaiement")
          );
          ...
          this.modePaiementModel.telephoneGenerique =
            modePaiementEditing.telephoneGenerique;
          this.modePaiementModel.typeModePaiementDesignation =
            modePaiementEditing.typeModePaiementDesignation;
