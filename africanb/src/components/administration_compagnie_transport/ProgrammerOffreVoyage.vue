...
import $ from 'jquery';
import { API_RECUPERER_LISTE_OFFRE_VOYAGE , API_OBENIR_JOUR_SEMAINE_PAR_OFFRE_VOYAGE , API_CREER_PROGRAMME_OFFRE_VOYAGE } from '../globalConfig/globalConstConfig'
...
export default {
    ...
    methods:{
        ...
        async enregistrerProgrammeOffreVoyage(){
            ...
            this.overlay = true ;
            await axios.post(API_CREER_PROGRAMME_OFFRE_VOYAGE, this.programmeObject).then((response) => {
        ...
        async recupererJourSemaineParOfrreVoyage(){
            ...
            await axios.post(API_OBENIR_JOUR_SEMAINE_PAR_OFFRE_VOYAGE, this.jourSemaineParOffreVoyage).then((response) => {
                if (response.status == 200) {
                    if (response.data.status.code != 800) {
                        this.errorMsg = response.data.status.message
                        $(".alert-error").fadeIn();
